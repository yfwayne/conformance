...
def test_write_large_lba(nvme0, nvme0n1, buf):
...
def test_write_max_namespace_size(nvme0, nvme0n1, buf, repeat):
...
def test_write_fua(nvme0, nvme0n1, buf):
    q = Qpair(nvme0, 8)
...
def test_write_bad_number_blocks(nvme0, nvme0n1):
    q = Qpair(nvme0, 8)
    mdts = nvme0.mdts//512
    buf = Buffer(mdts*512+4096)

    nvme0n1.write(q, buf, 0, mdts-1).waitdone()
    nvme0n1.write(q, buf, 0, mdts).waitdone()
    with pytest.warns(UserWarning, match="ERROR status: 00/02"):
        nvme0n1.write(q, buf, 0, mdts+1).waitdone()
    with pytest.warns(UserWarning, match="ERROR status: 00/02"):
        nvme0n1.write(q, buf, 0, mdts+2).waitdone()

    nlb = 1
    while nlb < mdts:
        nvme0n1.write(q, buf, 0, nlb).waitdone()
        nlb = nlb<<1

        
@pytest.mark.parametrize("ioflag", [0, 0x4000, 0x8000, 0xc000])
def test_write_valid(nvme0, nvme0n1, ioflag):
